...
import styles from "./WiFiSignalVisualizer.module.css";
...
const signalStrengthOffsets = {
  signal_strength_0_to_5_metres_offset: dBmToOffset(
    parseInt(modelData.signal_strength_0_to_5_metres)
  ),
  signal_strength_5_to_15_metres_offset: dBmToOffset(
    parseInt(modelData.signal_strength_5_to_15_metres)
  ),
  signal_strength_15_to_30_metres_offset: dBmToOffset(
    parseInt(modelData.signal_strength_15_to_30_metres)
  ),
  signal_strength_30_to_50_metres_offset: dBmToOffset(
    parseInt(modelData.signal_strength_30_to_50_metres)
  ),
};
...
function WifiHeatmapVisualizer() {
  ...
  console.log(
    "signalStrengthOffsets: " + JSON.stringify(signalStrengthOffsets)
  );
    ...
    <>
      <div style={{ height: "100vh", width: "100vw" }}>
        ...
        >
          ...
          <HeaderButton
          onClick={createAP}
          title="  Add a Router Point"
          name="  Add a Router Point"
          ...
          {addressPoints.map(([y, x], index) => (
            ...
            >
              <Popup>
                <div>
                  <p>
                    Router at [{y.toFixed(0)}, {x.toFixed(0)}]
                  </p>
                  <button
          ...
          <SVGOverlay bounds={bounds} style={{ pointerEvents: "none" }}>
            <defs>
              <radialGradient id="wifiGradient" cx="50%" cy="50%" r="50%">
                <stop
                  offset={`${signalStrengthOffsets.signal_strength_0_to_5_metres_offset}%`}
                  stopColor="green"
                  stopOpacity={0.8}
                />
                <stop
                  offset={`${signalStrengthOffsets.signal_strength_5_to_15_metres_offset}%`}
                  stopColor="yellow"
                  stopOpacity={0.8}
                />
                <stop
                  offset={`${signalStrengthOffsets.signal_strength_15_to_30_metres_offset}%`}
                  stopColor="orange"
                  stopOpacity={0.75}
                />
                <stop
                  offset={`${signalStrengthOffsets.signal_strength_30_to_50_metres_offset}%`}
                  stopColor="red"
                  stopOpacity={0.2}
                />
              </radialGradient>
